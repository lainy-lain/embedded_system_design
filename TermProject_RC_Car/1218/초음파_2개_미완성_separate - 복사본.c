...
void trig_pulse() // output trigger pulse to HC-SR04 trig pin at least 10us
{
...
void trig_pulse2() // output trigger pulse to HC-SR04 trig pin at least 10us
{
...
{
    unsigned long echo;
    trig_pulse2(); // give trig pulse to u_sonic sensor
    
    // while((PE_IDR & (1<<echoPin)) != (1<<echoPin)){
	while ( GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_1) != Bit_SET ) {
	// test
	printf("in 3\n");
	} // wait echo pin status turns HIGH
// != 0000 1000 0000 0000 (wait until bit11=1 )
