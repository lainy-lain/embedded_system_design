...
// DC모터 회전/속도관련 함수
void setDCMotor_goForward(void);
void setDCMotor_goForwardLeft(void);
void setDCMotor_goForwardRight(void);
void setDCMotor_goBackward(void);
void setDCMotor_goBackwardLeft(void);
void setDCMotor_goBackwardRight(void);
...
void executeOperation(void) {        
	...
	case OP_GO_FORWARD_LEFT : // 전진 + 좌회전
		if ( policy == RUNMODE_STOP || policy == RUNMODE_BACKWARD ) { 
			...
			setDCMotor_goForwardLeft(); // 전진 + 왼바퀴 OFF
		}
		break;
	case OP_GO_FORWARD_RIGHT : // 전진 + 우회전
		if ( policy == RUNMODE_STOP || policy == RUNMODE_BACKWARD ) { 
			...
			setDCMotor_goForwardRight(); // 전진 + 오른바퀴 OFF
		}
		break;
	...
	case OP_GO_BACKWARD_LEFT : // 후진 + 좌회전
		if ( policy == RUNMODE_STOP || policy == RUNMODE_FORWARD ) {
			...
			setServoMotor_rotateLeft(); // 왼쪽으로 조향
			setDCMotor_goBackwardLeft(); // 후진 + 왼바퀴 OFF
		}
		break;
	case OP_GO_BACKWARD_RIGHT : // 후진 + 우회전
		if ( policy == RUNMODE_STOP || policy == RUNMODE_FORWARD ) {
			...
			setServoMotor_rotateRight(); // 오른쪽으로 조향
			setDCMotor_goBackwardRight(); // 후진 + 오른바퀴 OFF
		}
		break;
...
void setDCMotor_goForwardLeft(void) {
    ...
    GPIO_ResetBits(GPIOE, GPIO_Pin_8 | GPIO_Pin_10 | GPIO_Pin_14);
...
void setDCMotor_goBackwardRight(void) {
    ...
    // 앞부분	
	// 8,10 왼바퀴 => 오른바퀴 OFF
    GPIO_ResetBits(GPIOE, GPIO_Pin_8 | GPIO_Pin_12 | GPIO_Pin_14);
