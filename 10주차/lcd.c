...
{
	/* 코드 설명
	LCD의 Register에 Command를 Write하기 위한 함수이다.
	Read Cycle이 아니므로 Read'를 High로 둔 후,
	Command Write Cycle의 Timing Diagram에 나와있는 대로
	D/C'를 Low, CS'를 Low,  WR'를 LOW로 설정해 주었다.
	Write를 수행한 후, CS'를 High로, WR'를 High로 다시 돌려놓아 주었다.
	*/
	...
	GPIO_ResetBits(GPIOC, GPIO_Pin_6);      // CS Low
	GPIO_ResetBits(GPIOD, GPIO_Pin_14);     // WR Low
	GPIO_Write(GPIOE, LCD_Reg);
	// TODO implement using GPIO_ResetBits/GPIO_SetBits
	GPIO_SetBits(GPIOC, GPIO_Pin_6);        // CS를 High로 다시 돌려놓기
	GPIO_SetBits(GPIOD, GPIO_Pin_14);       // WR를 High로 다시 돌려놓기
}

static void LCD_WR_DATA(uint16_t LCD_Data)
{
	/* 코드 설명
	LCD에 Data를 Write하기 위한 함수이다.
	Read Cycle이 아니므로 Read'를 High로 둔 후,
	Data Write Cycle의 Timing Diagram에 나와있는 대로
	D/C'를 High, CS'를 Low,  WR'를 LOW로 설정해 주었다.
	Write를 수행한 후, CS'를 High로, WR'를 High로 다시 돌려놓아 주었다.
	*/
	// TODO implement using GPIO_ResetBits/GPIO_SetBits
	GPIO_SetBits(GPIOD, GPIO_Pin_15);       // RD High
